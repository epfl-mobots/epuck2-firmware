...
#define MOTOR_ASEBA_WRITE(name)     static int name ## _old_period; static int name ## _old_enable;                             \
    if (vmVariables.name ## _pid_period != name ## _old_period || \
        ...
        if (vmVariables.name ## _pid_period && name ## _old_enable) {                                                            \
            ...
            if (vmVariables.name ## _pid_period > 0) {                                                                         \
                if (vmVariables.name ## _time_cst > 255L * 128 * vmVariables.name ## _pid_period) {                          \
                    ...
                    name ## _pid.time_cst = vmVariables.name ## _time_cst / \
                                            (vmVariables.name ## _pid_period * 128L);         \
                }                                                                                                           \
            }                                                                                                               \
    ...
    if (!name ## _pid.enable_p) {                                                                                              \
        vmVariables.name ## _speed_target = name ## _speed_to_pulse( \
            vmVariables.name ## _target_speed_ext);                   \
        name ## _pid.speed_t = vmVariables.name ## _speed_target;                                                           \
    }                                                                                                                   \
    *((long *) vmVariables.name ## _position_target) = name ## _position_to_pulse( \
        vmVariables.name ## _target_position_ext);  \
    if (vmVariables.name ## _pid_period <= 0) {                                                                              \
        pwm_set_duty(name ## _PWM, vmVariables.name ## _pwm_output); }

#define MOTOR_ASEBA_READ(name)   if (vmVariables.name ## _pid_period > 0    && \
                                     vmVariables.name ## _pid_enable) {              \
    ...
    if (name ## _pid.enable_p) {                                                                                           \
        ...
        vmVariables.name ## _target_speed_ext = name ## _pulse_to_speed( \
            vmVariables.name ## _speed_target);               \
    }                                                                                                               \
    vmVariables.name ## _speed_ext = name ## _pulse_to_speed(vmVariables.name ## _speed_read);                            \
    vmVariables.name ## _position_ext = name ## _pulse_to_position( \
        (long *) vmVariables.name ## _position);




#define MOTOR_LOAD_CONF(name)   name ## _pid.kp_s = vmVariables.name ## _kp_s = \
                                                        settings.name ## _kp_s;                       \
    ...
    name ## _pid.speed_max = vmVariables.name ## _speed_max = name ## _speed_to_pulse( \
                                 vmVariables.name ## _speed_max_ext);  \
...
#define MOTOR_ONE_STEP(name) motor_csp_step(&name ## _pid);                                                       \
    ...
    if ((settings.name ## _position_max != settings.name ## _position_min) && \
        !vmVariables.name ## _override) {   \
        int pos;                                                                                            \
